...
export class AuthService { 
    ...
    async register (data: Partial<User>) {
        ...
        if (userExists){
            throw new HttpException(400, 'User with this email already exists', 'email already exists' );
    ...
    async login (data: Partial<User>){
        ...
        if (!user){
            throw new HttpException (401, "invalid credentials", 'invalid credentials');
